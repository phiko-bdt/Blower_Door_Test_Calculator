...
장비는 **팬 모듈**(개구부에 설치), **제어 모듈**(조작·성적서 발행), 이 둘을 잇는
**연결선**으로 이루어진다.

![장비 구성 한눈에 — 팬 모듈·전원선·팬 커버·압력 측정 관·제어 모듈, 그리고 제어 모듈에서 나오는 팬 전원선(적·흑)·PWM 선(황)·압력 측정 관(청)](docs/images/items.png)

연결선은 세 가닥이다 — **전원선(적·흑)**, **PWM 신호선(황)**, **압력 측정
튜브(청)**. 차압센서는 양방향이라 청색 튜브를 반대로 꽂아도 측정은 되지만(부호만
반대), 가·감압 시험의 팬 방향에는 주의한다.
...
제어 모듈을 열면 뚜껑에 터치스크린, 왼쪽에 압력센서 표시기가 있다.

![제어 모듈 개방 — 터치스크린(첫 화면)과 압력센서 표시기](docs/images/control-module.png)

전원선·PWM선·측정관은 모두 제어 모듈 **측면**으로 들어간다. 위쪽 청색 포트가 압력
측정, 그 아래가 전원 인렛과 전원 스위치(‒O)다.

![제어 모듈 측면 — 냉각팬·압력 측정 포트(청)·전원 스위치(‒O)·전원 인렛](docs/images/control-module-side.png)

앱을 종료하면 라즈베리파이 바탕화면이 나온다. **[BlowerDoorTest]** 아이콘으로 다시
실행하고(더블클릭 → **[Execute]**), 발행된 성적서는 **[결과보고서]** 폴더에 쌓인다.

![바탕화면 — [결과보고서] 폴더와 [BlowerDoorTest] 실행 아이콘](docs/images/desktop.png)
...
- **필수 항목** (파란 테두리): 실내 체적(㎥), 팬 수량(쓰는 팬 **1·2**를 하나
  이상 체크 — 체크한 개수가 팬 수), 수행할 시험(감압/가압 중 하나 이상 선택).
- **시험 정보**(선택): 시험 목적·위치·의뢰자·시험자 등 성적서에 실릴 정보.
  비워 둬도 된다.
- 숫자 칸에 단위·글자를 넣으면 저장할 때 막는다(예: `424.21 ㎥` ✗ → `424.21` ○).
- 오른쪽 위 **[저장하고 시작]** 으로 다음 단계. 옆의 **[설정]** 은 시험
  기준값을 바꾼다(1-4 참고). (버튼을 위에 둬 화면 키보드가 떠도 안 가린다.)
...
> 바로 치며, 다 입력하면 **[완료]** 로 키보드를 닫는다. 숫자 칸의 키패드
> (0~9 · 점 · 음수 부호 · 지우기 한 줄)에는 완료 키가 없다 — 헤더의
> **[저장]** 류 버튼이 곧 확정이다. 설정 화면도 같다. 본문이 키보드에
> 가려지면 손가락으로 위아래로 쓸어(플릭) 스크롤할 수 있다.
...
### USB 로 복사
USB 저장소를 꽂으면 "USB 연결 감지됨" 알림이 잠깐 뜨고 **[USB 로 복사]**
버튼이 나타난다. 누르면 버튼이 **[복사 중…]** 으로 바뀌고(느린 USB 는 몇 초
걸릴 수 있다), 끝나면 성적서가 뜻있는 파일명으로 복사됐고 안전하게 뽑아도
된다는 안내가 뜬다. 안내가 뜨기 전에는 뽑지 말 것.
...
![① QR 스캔 후 — 폰이 단말 WiFi(BlowerDoor-Test)에 접속됨. '인터넷에 연결되어 있지 않음'은 정상(단말 WiFi에는 인터넷이 없음)](docs/images/wifi-connection.jpg)

![② 접속 후 — 폰에 뜬 성적서 목록. [받기]로 방금 시험한 성적서를, [이전 시험 성적서 보기]로 지난 성적서를 받는다](docs/images/report-download-page.jpg)
...
| 측정 가능 하한 | 15 Pa | 1~90 | 팬 최대에서 못 미치면 시험 불가 (목표보다 낮아야 함) |
| 압력 평활 창 | 20 점 | 1~60 | 조절 압력·수렴 판정 이동평균 표본 수 |
