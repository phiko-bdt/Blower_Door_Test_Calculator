...
FIELDS = [
    ("target_pressure", "목표 압력", "Pa",
     "측정 시작점. 이 압력을 맞춘 뒤 duty 를 낮춰가며 측정한다."),
    ("tolerance_percent", "수렴 허용 오차", "%",
     "목표 대비 몇 % 안에 들면 수렴으로 볼지. 10% · 목표 70 → ±7 Pa."),
    ("hold_seconds", "수렴 유지 시간", "초",
     "허용 오차 안에 이만큼 연속으로 머물러야 통과. 실패 판정에도 같은 시간을 쓴다."),
    ("max_pressure", "시험 가능 상한", "Pa",
     "팬을 최소로 낮춰도 이보다 높으면 압력을 제어할 수 없어 시험 불가."),
    ("measure_seconds", "지점 측정 시간", "초",
     "한 측정 지점에서 압력을 평균낼 시간."),
    ("settle_seconds_per_duty", "안정화 대기", "초/duty",
     "duty 를 1 옮길 때마다 기다릴 시간. duty 차가 클수록 오래 기다린다."),
    ("num_points", "측정 지점 수", "개",
     "목표 압력 지점부터 최저 지점까지 몇 등분해 측정할지."),
    ("low_pressure_warn", "저압 경고 기준", "Pa",
     "이보다 낮은 압력의 측정점에 경고를 표시한다 (회귀에는 포함)."),
]
